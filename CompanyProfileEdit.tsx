...
export default function App() {
  ...
  const isApprovedEmail = (email: string | null) => {
    if (!email) return false;
    const lower = email.toLowerCase().trim();
    return (
      APPROVED_ADMIN_EMAILS.includes(lower) || lower.endsWith("@bfgplt.com")
    );
  };
...
<div style="font-family:sans-serif; max-width:600px; border:1px solid #e2e8f0; border-radius:12px; padding:24px; color:#1e293b;">
  ...
  const handleLoginSubmit = async (e: React.FormEvent) => {
    ...
    // Check master default credentials (accepts admin_benaflash, admin@bfgplt.com, or admin)
    const isMasterAdminPass = password === "Benaflash@23**";
    const isMasterAdminUser =
      normUser === "admin_benaflash" ||
      normUser === "admin@bfgplt.com" ||
      normUser === "admin";
    ...
    try {
      const emailToAuth = username.includes("@") ? username : `${username}@bfgplt.com`;
      ...
      setLoginError(
        "Nama pengguna / e-mel atau kata laluan tidak sah! Sila semak semula atau guna butang 'Isi Kredensial Automatik'.",
      );
    }
  };
  ...
  const handleVerifyMfaCode = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const user = result.user;
      if (user?.email) {
        const role = await getUserRole(user.email, user.uid);
        if (role) {
          setIsLoggedIn(true);
          ...
          await signOut(auth);
          setLoginError(`Akses ditolak! Slogan keselamatan melarang emel Google (${user.email}) daripada mendaftar sistem BFG.`);
        }
      }
    } catch (err: any) {
                        ...
                        >
                          <div>
                            ...
                            <div className="p-5 space-y-2.5">
                              ...
                              <div className="grid grid-cols-2 gap-2 text-xs text-slate-500 pt-2 border-t border-slate-100">
                                <div>
                                  <span className="block text-[9px] uppercase font-bold text-slate-400">Nilai</span>
                                  <span className="font-semibold text-slate-700">{proj.value || 'Tender'}</span>
                                </div>
                                <div>
                                  <span className="block text-[9px] uppercase font-bold text-slate-400">Siap</span>
                                  <span className="font-semibold text-slate-700">{proj.completionDate}</span>
                                </div>
                              </div>
                            </div>
                          </div>
                      ...
                      >
                        <div>
                          ...
                          <input
                            type="text"
                            required
                            placeholder="admin_benaflash atau admin@bfgplt.com"
          ...
          <div className="fixed inset-0 bg-[#0F172A]/75 backdrop-blur-xs flex items-center justify-center p-4 z-50">
            ...
            >
              ...
              <div className="p-6 sm:p-8 space-y-4">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-xs font-semibold py-2">
                  <div className="p-3 bg-slate-50 border border-slate-100 rounded-lg">
